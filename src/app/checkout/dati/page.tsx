            ...
            <div className="space-y-1.5">
              ...
              <div className="relative">
                <select className="w-full bg-white border border-slate-200 px-3 py-2 text-sm appearance-none" value={data.comune || ''} onChange={(e) => onChange('comune', e.target.value)} required disabled={!data.provincia}>
                  ...
                  {(comuniPerProvincia[data.provincia] || []).map(c => <option key={c} value={c.toUpperCase()}>{c}</option>)}
                </select>
                <span className="material-symbols-outlined absolute right-3 top-2 pointer-events-none text-slate-400 text-base">expand_more</span>
              </div>
            </div>
                ...
                {accountType === 'privato' && (
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                    ...
                    <div className="md:col-span-2">
                      <label className="block text-xs font-bold uppercase tracking-wider text-[#516169] mb-2">Indirizzo di residenza *</label>
                      <input type="text" name="indirizzo" value={formData.indirizzo} onChange={handleChange} className="w-full" placeholder="Via Roma 1, 00100 Roma (RM)" required />
                    </div>
                    <div className="md:col-span-2">
